...
if test -n "${dictionaries}"; then
	perl -pi -e "s/<allowed_languages (.*)>.*<\/allowed_languages>/<allowed_languages \1>${dictionaries:-de_DE en_GB en_US}<\/allowed_languages>/" /etc/coolwsd/coolwsd.xml
else
	perl -pi -e "s/<allowed_languages (.*)>.*<\/allowed_languages>/<allowed_languages \1>de_DE en_GB en_US<\/allowed_languages>/" /etc/coolwsd/coolwsd.xml
fi
